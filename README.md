...
The Next.js app now lives at the **repository root**, so Vercel deploys it
directly on push to `main` — no Root Directory override needed. If the Vercel
project's Root Directory was previously pointed at `web`, reset it to the
repository root (`.`).

1. Vercel auto-detects the Next.js framework preset.
2. Add environment variables from `.env.example` (at minimum
...
| Blog posts (metadata + generated body) | `src/lib/blog.ts` + `src/lib/blog-content.ts` |
